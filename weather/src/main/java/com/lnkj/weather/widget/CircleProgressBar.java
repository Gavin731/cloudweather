...
import com.lnkj.weather.BuildConfig;
import com.lnkj.weather.R;
import com.lnkj.weather.utils.ChartUtils;
import com.lnkj.weather.utils.MiscUtil;

/**
 * @创建者 MuFeng-T
 * @创建时间 2020/3/31 21:28
 * @描述
 */
public class CircleProgressBar extends View {
    ...
    private int[] mGradientColors = {Color.parseColor("#33FF24"), Color.parseColor("#FFC62C"), Color.parseColor("#FF1111")};
    //当前进度，[0.0f,1.0f]
    ...
    private void drawLine(Canvas canvas) {
        for (int i = 0; i < 8; i++) {
            float mProgress = (i + 1) * 1.0f / 8 * mMaxValue;
            PointF mProgressPoint = ChartUtils.calcArcEndPointXY(mCenterPoint.x, mCenterPoint.y, mRadius + mArcWidth/2, 360 *
                    mProgress / mMaxValue, 269);
            float scale1 = mRadius * 1.0F / mArcWidth;
            float scale2 = mRadius * 1.0F / (mRadius - mArcWidth);
            //计算内圆上的点
            float disX = (scale1 * mProgressPoint.x + scale2 * mCenterPoint.x) / (scale1 + scale2);
            float disY = (scale1 * mProgressPoint.y + scale2 * mCenterPoint.y) / (scale1 + scale2);
            //计算外圆上的点
            ...
            canvas.drawLine(disX , disY, disX3, disY3, scalePaint);
        }
    }
