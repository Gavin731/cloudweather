 ...
 * @描述
 */
class HourDetailsFragment: BaseVMFragment<HourDetailsViewModel, WeatherFragmentHourDetailsBinding>() {
    ...
    override fun initView() {
        ...
        binding.ivShare.clickWithTrigger {
            ...
            HourDetailsShareActivity.launch(
                ...
                WeatherUtils.getWeatherBgByWeatherName(cityWeather?.weatherName)
                )
        }
    ...
    private fun initViewPager(myCityBean: MyCityBean?) {
        ...
        fragments.add(HourDetailsItemFragment.getInstance(calendar.time.time/1000, 2, myCityBean!!))
        calendar.time = Date()
        calendar.add(Calendar.DATE, 2)
        fragments.add(HourDetailsItemFragment.getInstance(calendar.time.time/1000, 3, myCityBean!!))
    ...
    private fun refreshData(myCityBean: MyCityBean?) {
        ...
        fragments.clear()
        val calendar: Calendar = GregorianCalendar()
        titleList.add(mapOf("day" to "今天", "date" to DateUtils.formatTime(Date(),
            DateUtils.PATTERN_14
        )))
        // 明天的时间
        calendar.time = Date()
        calendar.add(Calendar.DATE, 1)
        titleList.add(mapOf("day" to "明天", "date" to DateUtils.formatTime(calendar.time,
            DateUtils.PATTERN_14
        )))
        // 后天时间
        calendar.time = Date()
        calendar.add(Calendar.DATE, 2)
        titleList.add(mapOf("day" to "后天", "date" to DateUtils.formatTime(calendar.time,
            DateUtils.PATTERN_14
        )))
