...
open class BottomNavigationViewInner @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : BottomNavigationView(context!!, attrs, defStyleAttr) {
    // used for animation
...
// 1. get mMenuView
        ...
        } else { // if not record the mItemHeight, we need do nothing.
...
// 1. get mMenuView
        ...
        for (button in mButtons!!) {
            ...
            if (!visibility) { // if not record the font size, record it
            ...
            } else { // if not record the font size, we need do nothing.
...
// debug end
            } else { // haven't change the value. It means it was the first call this method. So nothing need to do.
...
// 2. get mButtons
     ...
     */
    open fun getMenuItemPosition(item: MenuItem): Int { // get item id
     ...
     * @return
     */
    open fun getOnNavigationItemSelectedListener(): OnNavigationItemSelectedListener? { // private OnNavigationItemSelectedListener mListener;
        return getField<OnNavigationItemSelectedListener>(BottomNavigationView::class.java, this, "selectedListener")!!
    }

    override fun setOnNavigationItemSelectedListener(listener: OnNavigationItemSelectedListener?) { // if not set up with view pager, the same with father
    ...
    open val bottomNavigationMenuView: BottomNavigationMenuView?
        get() {
            if (null == mMenuView) mMenuView = getField<BottomNavigationMenuView>(BottomNavigationView::class.java, this, "menuView")
            return mMenuView
        }
     ...
     * @return
     */
    open val bottomNavigationItemViews: Array<BottomNavigationItemView>?
        get() {
    ...
    open fun getBottomNavigationItemView(position: Int): BottomNavigationItemView? {
        return bottomNavigationItemViews?.get(position)
    }
     ...
     * @param height in px
     */
    open fun setItemHeight(height: Int): BottomNavigationViewInner? { // 1. get mMenuView
        val mMenuView = bottomNavigationMenuView
     ...
     * not cause a leak.
     */
    private class BottomNavigationViewExOnPageChangeListener(bnve: BottomNavigationViewInner) : OnPageChangeListener {
     ...
     */
    private class MyOnNavigationItemSelectedListener internal constructor(viewPager: ViewPager, bnve: BottomNavigationViewInner, smoothScroll: Boolean, listener: OnNavigationItemSelectedListener?) : OnNavigationItemSelectedListener {
...
// detect navigation tab changes when the user clicking on navigation item
    ...
    init {
        val a = ThemeEnforcement.obtainTintedStyledAttributes(context!!, attrs,
                R.styleable.BottomNavigationView,
                defStyleAttr, R.style.Widget_Design_BottomNavigationView,
                *intArrayOf(R.styleable.BottomNavigationView_itemTextAppearanceInactive,
